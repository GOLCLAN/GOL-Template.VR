...
// *
// *	Usage:
// *		[player, gol_flag] call GVL_fnc_Teleport;;
// *		player addAction ["Teleport1",{[player,gol_flag] call GVL_Fnc_Teleport}];
// *
// *	Parameters:
// *		0:	String:		Unit
// *		1:	String:		Target
...
if (_dest isKindOf "LandVehicle" || _dest isKindOf "Air" || _dest isKindOf "Ship") Then {
	...
	if (_freeCargoPositions > 0) Then {
		["Teleported to MHQ's cargo", 5] call GVL_Fnc_Hint;
		_unit moveInCargo _dest;
	} else {
		if (!isengineOn _dest || (speed _dest) == 0) Then {
			["The MHQ is stationary", 5] call GVL_Fnc_Hint;
			[] call _Teleportar;
		} else {
			["The MHQ is filed up & on the move, wait until they stop and try again", 5] call GVL_Fnc_Hint;
		};
	};
} else {
	[] call _Teleportar;

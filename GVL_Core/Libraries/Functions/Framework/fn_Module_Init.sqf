...
// ================================================================
	...
	if (isClass(configFile>>"CfgPatches">>"AGM_Interaction")) then {
		...
		_module setVariable ["EnableTeamManagement", true, true];
			GVL_Module_Interaction = _module;
		[] spawn {
			publicVariable "GVL_Module_Interaction";
		};
	};
...
// ================================================================
	...
	if (isClass(configFile>>"CfgPatches">>"AGM_Logistics")) then {
		...
		_module setVariable ["MaxDamageRepair", 0.9, true];
			GVL_Module_Logistic = _module;
			AGM_Drag_MaxWeight = 100000;
		[] spawn {
			publicVariable "GVL_Module_Logistic";
...
// ================================================================
	...
	if (isClass(configFile>>"CfgPatches">>"AGM_Medical")) then {
		...
		_module setVariable ["DisableScreams", false, true];
			GVL_Module_Medical = _module;
		[] spawn {
			publicVariable "GVL_Module_Medical";
		};
	};
...
// ================================================================
	...
	if (isClass(configFile>>"CfgPatches">>"AGM_Respawn")) then {
		...
		_module setVariable ["SavePreDeathGear", false, true];
			GVL_Module_Respawn = _module;
		[] spawn {
			publicVariable "GVL_Module_Respawn";
		};
	};
	...
	if (isClass(configFile>>"CfgPatches">>"AGM_Map")) then {
		...
		_module setVariable ["HideAiGroups", false, true];
			GVL_Module_BFT = _module;
			AGM_Map_EveryoneCanDrawOnBriefing = false;
		[] spawn {
			publicVariable "GVL_Module_BFT";
...
// ================================================================
	...
	if (isClass(configFile>>"CfgPatches">>"A3_Modules_F_Curator")) then {
		...
		_module setvariable ["bis_fnc_curatorSystem_points",1000];
			...
			GVL_ZeuzModuleAdminLogged = _module;
			GVL_Gamelogic = _moduleGroup createUnit["Logic",[0,0,0],[],0,"NONE"];
		[] spawn {
			publicVariable "GVL_ZeuzModuleAdminLogged";
			publicVariable "GVL_Gamelogic";
		};
	};
//	[GVL_Gamelogic] call GVL_Fnc_CacheObjects;
//	[GVL_Gamelogic] call GVL_Fnc_CacheVehicles;

	if (["Cache","Enabled"] call GVL_Fnc_GetConfig == 1) then {
//		[GVL_Gamelogic] call GVL_Fnc_FPS_Manager;
	};

	if (["Lightning"] call GVL_Fnc_GetConfig == 1) then {
		GVL_Lightning_Active = true;
	};
...
private ["_DebugName"];
_DebugName = "GVL-Modules";
scriptName _DebugName;

["FRAMEWORK MODULES INITIALIZED",[_DebugName,__FILE__,__LINE__],"log"] call GVL_Fnc_DebugLog;

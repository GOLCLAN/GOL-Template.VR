...
// ================================================================
	...
	// ===============================================
		...
		if ((!isNil "HeadlessController") && (HeadlessVariable)) Then {
			["Headless Checking INITIALIZED SUCCESSFULLY",[_DebugName,__FILE__,__LINE__],"log"] call GVL_Fnc_DebugLog;
			if (serverCommandAvailable "#logout") then {
				systemChat "Loading Headless Client - SUCCESSFULLY";
			};
		} else {
			["Headless Checking initialize failed",[_DebugName,__FILE__,__LINE__],"log"] call GVL_Fnc_DebugLog;
			if (serverCommandAvailable "#logout") then {

...
// ====================================================================================
// Menu shown/hidden
// disable due to being a bit wonky
if(abs (f_cam_menuShownTime - time) <= 0.1 && !f_cam_menuShown) then {
	[true] spawn f_fnc_showMenu;
};
// disable due to being a bit wonky
if(abs (f_cam_menuShownTime - time) >= 1 && f_cam_menuShown) then {
...
if(f_cam_mode == 3) then {
	...
	// W
	if(f_cam_freecam_buttons select 0) then {
	...
	// S
	if(f_cam_freecam_buttons select 1) then {
		if(f_cam_shift_down) then
		{
			_mY = -_accelshift;
		} else {
	...
	// A
	if(f_cam_freecam_buttons select 2) then {
		if(f_cam_shift_down) then
		{
			_mX = -_accelshift;
		} else {
	...
	// D
	if(f_cam_freecam_buttons select 3) then {
		if(f_cam_shift_down) then
		{
			_mX = _accelshift;
		} else {
	...
	// Q
	if(f_cam_freecam_buttons select 4) then {
	...
	// Z
	if(f_cam_freecam_buttons select 5) then {
		_scroll = -1*((sqrt _height)/2)*_delta;
		if (abs _scroll < 0.1) then {
			if (_scroll < 0) then { _scroll = -0.1;}
			else { _scroll = 0.1;};

...
_this spawn {
	GVL_Spectator_Enabled = true;
	_unit = [_this, 0, player,[objNull]] call BIS_fnc_param;
	_oldUnit = [_this, 1, player,[objNull]] call BIS_fnc_param;
	_forced = [_this, 2, true,[false]] call BIS_fnc_param;
	if(isNil "f_cam_isJIP") then { f_cam_isJIP = true; };
...
//		deleteVehicle _unit;
	...
	f_cam_fired = [];
	{
		_event = _x addEventHandler ["fired",{f_cam_fired = f_cam_fired - [objNull];

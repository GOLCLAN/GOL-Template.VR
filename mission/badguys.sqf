...
/*	
	...
	Step 3. Create another trigger of any size, call it zone1trigger then in the condition enter (getPos ((list zone1trigger) select 0) select 2) <= 1 and finally in the act enter ZONE1 = 1;
	...
	Step 7. While in the editor if you press escape you will see a large window called "Debug Console" you can paste copy snippets into this box and click "Local Exec" this will then run the code.
			...
			///copyToClipboard format ["['zone1', %1, 'UP', 0] call Fnc_SpawnUnit;",getPosATL player]; 
			///copyToClipboard format ["['zone1', %1, 'UP'] call Fnc_SpawnSniper;",getPosATL player]; 
			///copyToClipboard format ["['Zone1Group1', %1, 1, 'MOVE', 'LINE', 'RED', 'AWARE', 'NORMAL'] call Fnc_wayPoint;",getPosATL player];
			///["Zone1Group1", [10785.7,8221.48,0.00154114], 1, "MOVE", "LINE", "YELLOW", "AWARE", "NORMAL"] call Fnc_wayPoint;
			///onMapSingleClick "copyToClipboard format ['%1',_pos]";
			///CopyToClipboard format ["['Land_fortified_nest_big_EP1', %1, %2 - 180, 2] call Fnc_SpawnBunker;",getPosATL player,getDir player];
			///CopyToClipboard format ["['RDS_KORD_high_CSAT', %1, %2] call Fnc_StaticWeapon;",getPosATL player,getDir player];
			///CopyToClipboard format ["['O_SD_Offroad_01_armed', %1, %2, 1, 2] call Fnc_SpawnVehicle;",getPosATL player,getDir player];
			
	---[DYNAMIC-UNITS]---
	
	["zone1", 10, 125, "buildings"] call Fnc_Populate;
	["zone1", 18, 225, "patrols"] call Fnc_Populate;
	
	Spawn Marker Name, Number of Units, Spawn Radius, Populate Type buildings, patrols		
	
	---[STATIC-UNITS]---
	
	["zone1", [10843.3,8158.47,0.00177002], "UP", 1] call Fnc_SpawnUnit;
	["zone1", [10812.9,8190.88,0.00106812], "UP", 0] call Fnc_SpawnUnit;
	["zone1", [10796.8,8177.33,0.00178528], "UP", 0] call Fnc_SpawnUnit;
	["zone1", [10777.3,8100.34,0.0014801], "UP", 0] call Fnc_SpawnUnit;
	
	Spawn Marker Name, Position, Stance, Leader 0/1	
	
	---[STATIC-SNIPERS]---
	
	["zone1", [10843.3,8158.47,0.00177002], "UP"] call Fnc_SpawnSniper;
	
	Spawn Marker Name, Position, Stance
	
	---[STATIC-ANTIAIR-TROOPS]---
	
	["zone1", [10843.3,8158.47,0.00177002], "UP"] call Fnc_SpawnAntiAir;
	
	Spawn Marker Name, Position, Stance
	
	---[MOBILE-UNITS]---
	
	["Zone1Group1", "zone1", 5] call Fnc_SpawnGroup;
	sleep 0.3;
	["Zone1Group1", [10785.7,8221.48,0.00154114], 1, "MOVE", "STAG COLUMN", "YELLOW", "SAFE", "LIMITED"] call Fnc_wayPoint;
	["Zone1Group1", [10877.9,8095.71,0.00172424], 2, "MOVE", "STAG COLUMN", "YELLOW", "SAFE", "LIMITED"] call Fnc_wayPoint;
	["Zone1Group1", [10785.7,8221.48,0.00154114], 3, "CYCLE", "STAG COLUMN", "YELLOW", "SAFE", "LIMITED"] call Fnc_wayPoint;
	sleep 2;
	...
	moving STAG COLUMN. After this we specify a colour which is basically a mode for how the AI will react when they're engaged. Next We have specified SAFE this is how the AI will hold its weapon while moving and 
	...
	[10, "zone1", 250] call Fnc_SpawnCivilians;
	...
	---[BUNKERS]---	
	["Land_fortified_nest_big_EP1", [1844.85,5725.56,0.00143862], 173, 4] call Fnc_SpawnBunker;
	...
	["rhsusf_m1025_w_m2", [1844.85,5725.56,0.00143862], 88, 1, 3] call Fnc_SpawnVehicle;
	...
	["rhsusf_m1025_w_m2", [22923.9,21013.2,0.00156403], 163, 0, 3] call Fnc_SpawnVehicle; sleep 2;
	["VehTeam", [23757.8,20331.1,0.00150299], 1, "MOVE", "WEDGE", "YELLOW", "AWARE", "LIMITED"] call Fnc_wayPoint;
	["VehTeam", [22923.9,21013.2,0.00156403], 2, "MOVE", "WEDGE", "YELLOW", "AWARE", "LIMITED"] call Fnc_wayPoint;
	["VehTeam", [22923.9,21013.2,0.00156403], 3, "CYCLE", "WEDGE", "YELLOW", "AWARE", "LIMITED"] call Fnc_wayPoint;
	
	--[PLANES]--
	
	["B_Plane_CAS_01_F", [6451.74,5437.13,0.00148487], 0] call Fnc_SpawnPlane;
	...
	["VehTeam", [12738,12254.1,0.00131989], 1, "MOVE", "WEDGE", "YELLOW", "AWARE", "LIMITED"] call Fnc_wayPoint;
	["VehTeam", [10459.2,12414.8,0.00146484], 2, "MOVE", "WEDGE", "YELLOW", "AWARE", "LIMITED"] call Fnc_wayPoint;
	["VehTeam", [12738,12254.1,0.00131989], 3, "CYCLE", "WEDGE", "YELLOW", "AWARE", "LIMITED"] call Fnc_wayPoint;
	
	--[STATIC-WEAPONS]---	
	["RDS_KORD_high_CSAT", [1897.04,5729.67,0.00145102], 191] call Fnc_StaticWeapon;
	...
	--[RANDOM-POSITIONS]---	
	[officer_01, [1280,7583.55,1.29939], [1419.49,7728.37,10.3551], [1298.1,7648.26,3.84938]] call Fnc_RandomPosition;

...
_EnableMarking = True;
if (IsNil "NEKY_Mortars") then {NEKY_Mortars = []};
...
if ( (isNil "_Mortar") or (TypeName _Mortar == "STRING") ) then 
{
	...
	} else {systemChat "Mortar is destroyed or non-existant"};
	_OffMap = False;
};
...
While {((Alive _Mortar) && (Alive _Unit) && (_Unit in _Mortar)) or (_OffMap)} do
{
	...
	if (_Scanner) then 
	{
		...
		if (_ScanVehicles) then
		{
			{ if ( (((Side _Unit) knowsAbout _x) > 3.95) && !((side _x) == (side _Unit)) && ((side _Unit) GetFriend (side _x) < 0.6) && (_Unit distance _x > _MinRange) && (_Unit distance _x < _MaxRange) && (Alive _x) && (((getposATL _x) select 2) < 5) ) then {_Units PushBack _x}; } ForEach (list _Zone);
		} else {
			{ if ( (((Side _Unit) knowsAbout _x) > 3.95) && !((side _x) == (side _Unit)) && ((side _Unit) GetFriend (side _x) < 0.6) && (_Unit distance _x > _MinRange) && (_Unit distance _x < _MaxRange) && (_x isKindOf "CAManBase") && (Alive _x) && (((getposATL _x) select 2) < 5) ) then {_Units PushBack _x}; } ForEach (list _Zone);
		};
	
		sleep 0.5;
		if (Count _Units == 0) exitWith {False; _FriendlyNear = False};
		_Position = GetPosATL (_Units call BIS_FNC_SelectRandom);
		if ( (_Avoid) && ( {((side _x == side _Unit)) && (_Unit distance _x > _MinRange) && (_Unit distance _x < _MaxRange) && ((side _Unit) getFriend (side _x) > 0.6) && ((_Position distance _x) < (_NewInaccuracy)) && (Alive _x) } count (list _Zone) > 0) ) then {_FriendlyNear = True} else {_FriendlyNear = False};
	};

	if ((!(_FriendlyNear) && (Count _Units > 0)) or (_OffMap)) then 
	{
		// Firing modes
		_Index = 0;
		...
		SystemChat "Friendly too close, rescanning"; 
		sleep 10;
	};
	if ((_OffMap) or !(_Scanner)) exitWith {systemChat "Off map mode or not in auto mode, exiting script"};
	DeleteVehicle _Zone;
};
